...
object Tools {
  def length(x: Float, y: Float): Float = math.sqrt(x * x + y * y).toFloat

  def length(v: (Float, Float)): Float = length(v._1, v._2)

  def length(x: Double, y: Double): Double = math.sqrt(x * x + y * y)
